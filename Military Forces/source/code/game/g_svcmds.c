...
*/
	...
	if (Q_stricmp (cmd, "bot_remove") == 0)
	{
		Bot_Remove_f();
		return true;
	}

	if (Q_stricmp (cmd, "bot_waypoint_add") == 0)
	{
		Bot_WaypointAdd_f();
		return true;
	}

	if (Q_stricmp (cmd, "bot_waypoint_save") == 0)
	{
		Bot_WaypointSave_f();
		return true;
	}

	if (Q_stricmp (cmd, "bot_waypoint_load") == 0)
	{
		Bot_WaypointLoad_f();
		return true;
	}

	if (Q_stricmp (cmd, "bot_add_circling") == 0)
	{
		Bot_AddCircling_f();
		return true;
	}
