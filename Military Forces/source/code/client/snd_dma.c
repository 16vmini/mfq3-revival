...
void S_BeginRegistration( void ) {
	...
	if (s_numSfx == 0) {
		...
		S_RegisterSound("sound/feedback/hit.wav", false);		// changed to a sound in mfdata
	}
}
...
void S_AddLoopingSound( int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfxHandle ) 
{
	...
	if ( !sfx->soundLength ) {
		// Revival fix: a zero-length / corrupt sound in the archival paks
		// (e.g. sound/feedback/hit.wav) must not be fatal. Warn once and skip.
		Com_Printf( S_COLOR_YELLOW "%s has length 0 - skipping looping sound\n", sfx->soundName );
		return;
	}
