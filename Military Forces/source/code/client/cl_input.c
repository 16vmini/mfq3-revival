...
void CL_MouseMove( usercmd_t *cmd ) {
	...
	if ( (in_mlooking || cl_freelook->integer) && !in_strafe.active ) {
		float pitchScale = m_pitch->value;
		// MFQ3: optionally invert pitch ONLY while piloting an aircraft (plane or
		// helo), so flight feels like a stick (push forward = nose down / pull
		// back = climb) without touching ground vehicles, boats, or on-foot aim.
		// Quake's view-pitch convention is built for an FPS, so aircraft always
		// felt "backwards" to stick flyers; this scopes the fix to aircraft only.
		if ( m_invertAircraftPitch->integer && cl.snap.valid &&
			 cl.snap.ps.pm_type == PM_VEHICLE ) {
			int veh = (int)Cvar_VariableValue( "cg_vehicle" );
			if ( veh >= 0 && veh < bg_numberOfVehicles &&
				 ( availableVehicles[veh].cat & (CAT_PLANE | CAT_HELO) ) ) {
				pitchScale = -pitchScale;
			}
		}
		cl.viewangles[PITCH] += pitchScale * my;
	} else {
